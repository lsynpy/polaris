...
class Multiselect<T extends Item> {
  ...
  clickItem(event: MouseEvent, item: T) {
    ...
    const pivotIndex = this.pivotKey
      ? toValue(this.items).findIndex((i) => i.key === this.pivotKey.value)
      : -1;
  ...
  move(event: KeyboardEvent, delta: number) {
    ...
    if (!event.shiftKey || pivotIndex < 0) {
      ...
      const items = toValue(this.items);
...
export default function useMultiselect<T extends Item>(
  items: MaybeRefOrGetter<T[]>,
  options?: MultiselectOptions
) {
