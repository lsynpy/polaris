...
import type { Album, AlbumKey, BrowserEntry, Song } from "./api/dto";
...
document.querySelectorAll("#dnd-blank, #dnd-drag").forEach((e) => {
  e.remove();
});
...
dragElement.className =
  "w-fit bg-ls-0 dark:bg-ds-950 px-2 py-1 rounded-md text-sm border-2 border-accent-200 dark:border-accent-800 text-accent-700 ring-2 ring-ls-0 dark:ring-ds-950";
dragElement.style.pointerEvents = "none";
document.body.appendChild(dragElement);
yeet(dragElement);

const storage: Ref<DnDPayload | undefined> = ref(undefined);
...
function updateDrag(_event: DragEvent) {
  dragElement.style.left = `${mouseX.value}px`;
  dragElement.style.top = `${mouseY.value}px`;
}

function endDrag(_event: DragEvent) {
  yeet(dragElement);
