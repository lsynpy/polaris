    ...
    <div v-else class="flex flex-col min-h-0">
      ...
      <div class="-mx-8 px-8 -mt-3 pt-3 min-h-0 overflow-y-auto">
        <div class="flex items-center gap-6 mt-11 xl:mt-0 mb-8">
          <div
            class="shrink-0 basis-24 rounded-lg shadow-md"
            :class="[
              albumURL
                ? 'cursor-pointer origin-center transition-all ease-out duration-100 hover:opacity-90 hover:scale-125'
                : '',
            ]"
        ...
        <div
          class="divide-y divide-ls-100 dark:divide-ds-700 even:*:bg-ls-0 odd:*:bg-ls-50 dark:even:*:bg-ds-700/30 dark:odd:*:bg-ds-800/30"
        >
          <SongField
...
const artworkURLSmall = computed(() =>
  song.value?.artwork ? makeThumbnailURL(song.value.artwork, 'small') : undefined
);
