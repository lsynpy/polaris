  ...
  <div>
    <div v-for="(event, index) in events" :key="index">
      <div class="flex gap-x-4 items-center">
        <div
          :class="event.isMainRelease ? 'h-24 w-24 rounded-lg' : 'h-12 w-12 mx-6 rounded-full'"
          class="shrink-0 overflow-hidden shadow-md cursor-pointer origin-center transition-all ease-out duration-100 hover:opacity-90 hover:scale-125"
          @click="onAlbumClicked(event.album)"
        >
          ...
          <div class="mr-8 flex flex-col text-sm text-ls-500 dark:text-ds-400">
            <p class="whitespace-pre-line leading-6">
              ...
              <span v-if="event.albumOwners?.length">
                <span
                  v-for="(ownerArtist, ownerIndex) of event.albumOwners"
                  :key="ownerIndex"
                  class="inline-flex"
                >
                  <span
                    class="cursor-pointer underline text-accent-600 dark:text-accent-700"
                    @click="onArtistClicked(ownerArtist)"
                    v-text="ownerArtist"
                  />
                  <span v-if="ownerIndex == event.albumOwners.length - 2">&nbsp;&&nbsp;</span>
                  <span v-else-if="ownerIndex < event.albumOwners.length - 1">,&nbsp;</span>
                </span>
                <span>'s&nbsp;</span>
              </span>
              ...
              <span v-if="event.albumCollaborators?.length">
                <span>&nbsp;with&nbsp;</span>
                <span
                  v-for="(collabArtist, collabIndex) of event.albumCollaborators"
                  :key="collabIndex"
                  class="inline-flex"
                >
                  <span
                    class="cursor-pointer underline text-accent-600 dark:text-accent-700"
                    @click="onArtistClicked(collabArtist)"
                    v-text="collabArtist"
                  />
                  <span v-if="collabIndex == event.albumCollaborators.length - 2"
                    >&nbsp;&&nbsp;</span
                  >
                  <span v-else-if="collabIndex < event.albumCollaborators.length - 1">,&nbsp;</span>
                </span>
              </span>
            </p>
          </div>
...
const events = computed(() => {
  ...
  for (const album of props.albums) {
    ...
    const multiRole =
      [numPerformerCredits, numComposerCredits, numLyricistCredits].filter((n) => n > 0).length > 1;
    const isMainArtist = album.main_artists.includes(props.artist);
    const albumOwners = isMainArtist
      ? undefined
      : album.main_artists.filter((a) => a !== props.artist && !isFakeArtist(a));
...
function onAlbumClicked(album: AlbumHeader) {
  router.push(makeAlbumURL(album.main_artists, album.name));
}
</script>
