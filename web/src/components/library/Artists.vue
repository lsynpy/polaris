<template>
  <div class="flex flex-col whitespace-nowrap">
    <PageHeader v-model:view-mode="roleFilter" title="Artists" :view-modes="roles" />
    ...
    <div v-if="artists.length" class="grow min-h-0 flex flex-col">
      ...
      <ArtistGrid
        ...
        ref="list"
        class="grow min-h-0 -mr-4 pr-4"
        :artists="filtered"
      />
      <div v-else class="grow flex mt-40 justify-center text-center">
        <BlankStateFiller icon="filter_alt_off"> No artists match this filter. </BlankStateFiller>
      </div>
    </div>
    ...
    <Error v-else-if="error"> Something went wrong while listing artists. </Error>
