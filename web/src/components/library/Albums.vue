<template>
  <div class="flex flex-col whitespace-nowrap">
    <PageHeader v-model:view-mode="viewMode" title="Albums" :view-modes="viewModes" />
    ...
    <div v-if="albums.length" class="grow min-h-0 flex flex-col">
      ...
      <div v-if="!filtered.length" class="grow flex mt-40 justify-center text-center">
        <BlankStateFiller icon="filter_alt_off"> No albums match this filter. </BlankStateFiller>
      </div>

      <AlbumGrid
        ref="grid"
        class="-m-4 p-4 overflow-y-auto"
        :albums="filtered"
        :show-artists="true"
      >
        <template #footer>
    ...
    <Error v-else-if="error"> Something went wrong while listing albums. </Error>
...
import { computed, type Ref, ref, type ShallowRef, toRaw, useTemplateRef, watch } from 'vue';
...
import PageHeader, { type PageViewMode } from '@/components/basic/PageHeader.vue';
