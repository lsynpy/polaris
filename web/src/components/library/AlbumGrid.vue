<template>
  <div
    ref="viewport"
    class="overflow-y-auto -m-4 p-4 mb-0 pb-0"
    tabindex="-1"
    :style="{
      gap: `${gapSize}px`,
    }"
  >
    <div class="relative" :style="{ height: `${contentHeight}px` }">
      ...
      />
      <AlbumGridCell
        v-for="(album, index) of virtualAlbums"
        :key="album.name + album.main_artists.join('')"
        :album="album"
        :size="cellSize"
        :show-artists="showArtists"
        data-pw="album"
...
const contentHeight = computed(
  () =>
    Math.ceil(trimmedAlbums.value.length / numColumns.value) * (itemHeight.value + gapSize.value)
);

const firstVirtual = computed(
  () => Math.floor(scrollY.value / (itemHeight.value + gapSize.value)) * numColumns.value
);
const numVirtualItems = computed(
  () => Math.ceil(1 + viewportHeight.value / (itemHeight.value + gapSize.value)) * numColumns.value
);
