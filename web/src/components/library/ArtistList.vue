    ...
    >
      ...
      >
        ...
        <div class="basis-fit shrink min-w-0 pr-8 flex flex-col">
          <span
            class="cursor-pointer font-semibold overflow-hidden text-ellipsis text-ls-700 dark:text-ds-300 hover:text-accent-600 hover:underline"
            :class="listMode == 'fixed' ? 'text-sm' : ''"
            :style="proportionalStyle[item.data.name]"
            @click="router.push(makeArtistURL(item.data.name))"
          >
            {{ item.data.name }}
          </span>
