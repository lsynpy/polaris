<template>
  <div class="relative group flex gap-4">
    <div
      class="absolute -left-1 right-0 bottom-3.5 -top-1 rounded-sm group-hover:bg-ls-100 group-hover:dark:bg-ds-900 outline-dotted outline-accent-500 dark:outline-accent-700"
      :class="[focused ? 'outline-1' : 'outline-0']"
    />
    <div
      class="z-10 w-6 text-right"
      :class="[
        selected ? 'text-accent-500 dark:text-accent-700' : 'text-ls-500 dark:text-ds-400',
        song.track_number ? '' : 'opacity-0',
      ]"
      v-text="`${song.track_number}.`"
    />
    <div
      class="z-10 grow mb-2 pb-2 border-ls-200 dark:border-ds-700"
      :class="isLast ? 'border-0' : 'border-b'"
    >
      <span
      ...
      >
        ...
        <span v-for="(artist, index) of artists" :key="index">
          <span
            ...
            @click="onArtistClicked($event, artist)"
            v-text="artist"
          />
