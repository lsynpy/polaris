<template>
  <div class="flex flex-col">
    ...
    <div v-show="songPaths.length" class="flex flex-col min-h-0">
      <SectionTitle
        :label="`${songPaths.length} ${pluralize('song', songPaths.length)} found`"
      ...
      >
        ...
        <template #right>
          <!-- TODO tooltips -->
          <Switch
            v-model="preferences.searchResultsDisplayMode"
            :items="[
      ...
      <SongList
        v-model="songPaths"
        :compact="preferences.searchResultsDisplayMode == 'compact'"
        invert-stripes
      />
    </div>
    ...
    <div v-else class="grow flex items-start mt-40 justify-center text-center">
      <BlankStateFiller icon="search_off"> No songs found for this query. </BlankStateFiller>
    </div>

    <SidePanel v-model="showHelp">
      <div class="flex flex-col gap-4 text-ls-900 dark:text-ds-200 text-sm">
        <div class="text-2xl font-semibold leading-6 text-ls-700 dark:text-ds-200 mb-8">
          Search Syntax
        </div>
        ...
        <p>
          Searching for <span class="font-medium text-accent-600 font-mono">sonata</span> will find
          any song with "sonata" in its title, artist, file path, album name etc.
        </p>

        <p>
          Search terms containing spaces should be wrapped with quotes, for example
          <span class="font-medium text-accent-600 font-mono">"moonlight sonata"</span>. Spaces can
          also be omitted altogether for the same result (ie.
          <span class="font-medium text-accent-600 font-mono">moonlightsonata</span>).
        </p>
        ...
        <p>
          Partial matches for text fields can be expressed with the % operator. For example,
          <span class="font-medium text-accent-600 font-mono">composer % bac</span> would returns
          pieces with J.S. Bach or Jacques Offenbach as their composer.
        </p>
        <p>
          Exact matches can be expressed with the = operator. For example,
          <span class="font-medium text-accent-600 font-mono">label = ninja</span> would return
          songs that have a "Ninja Tuna" label.
        </p>
        <p>
          For number fields (track number, disc number, year), comparisons using
        ...
        <p>
          Queries can be combined with ||, && and !! to express logical OR, AND and NOT operations.
          Parenthesis can be used to alter precedence.
        </p>

        <p>For example:</p>
        <ul class="ml-8 list-disc">
          <li>
            <span class="font-medium text-accent-600 font-mono"> sonata !! moonlight </span>
          </li>
          <li>
        ...
        <p>
          Queries separated by spaces are implicitly joined by &&. For example searching for
          <span class="font-medium text-accent-600 font-mono">sonata beethoven</span> will list
          pieces with "sonata" appearing in any field and "beethoven" appearing in any field.
        </p>
