<template>
  <div>
    <label
      v-if="label"
      :for="id"
      class="block text-sm font-medium leading-6 text-ls-900 dark:text-ds-0"
    >
      {{ props.label }}
    </label>
    <div :class="{ 'mt-2': label }" class="relative rounded-md shadow-sm">
      ...
      <input
        ...
        :required="required"
        :class="inputClass"
        class="w-full rounded-md border-0 py-2 pr-8 shadow-sm text-sm leading-6 ring-1 ring-inset focus:ring-2 focus:ring-inset dark:bg-white/5 text-ls-900 dark:text-ds-0 placeholder:text-ls-400 dark:placeholder:text-ls-400 ring-ls-300 dark:ring-white/10 focus:ring-accent-600 dark:focus:ring-accent-600"
      />
      <div
