<template>
  <div
    class="mt-11 mb-8 border-ls-200 dark:border-ds-700 flex flex-col xl:flex-row gap-x-8 gap-y-4 xl:items-center justify-between"
  ...
  >
    <div class="min-w-0 flex flex-col items-stretch gap-y-4 xl:flex-row xl:items-center">
      <slot name="jumbo">
        <div class="min-w-0 flex items-center">
          <h3
            class="text-4xl text-ls-500 dark:text-ds-400 whitespace-nowrap overflow-hidden text-ellipsis"
          >
            {{ title }}
          </h3>
      ...
      <SwitchText
        ...
        v-model="viewMode"
        class="h-9 items-center xl:ml-6 xl:pl-6 xl:border-l border-ls-200 dark:border-ds-700"
        :items="viewModes"
      />
    </div>
    <slot name="right" />
    ...
    >
      <div class="italic text-sm text-ls-400 dark:text-ds-500" v-text="caption" />
    </div>
    <div v-if="actions?.length" class="flex gap-2">
      <Button
        v-for="action in actions"
        :key="action.testID || actions.indexOf(action)"
        :label="action.label"
        :icon="action.icon"
