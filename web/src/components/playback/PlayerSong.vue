<template>
  <div class="flex flex-col gap-4">
    <div
      class="flex justify-center items-center px-2 text-md lg:text-sm text-ls-900 dark:text-ds-200 gap-1"
    ...
    >
      <div v-if="artists && !miniPlayer" class="overflow-hidden text-ellipsis">
        <span v-for="(artist, index) in artists" :key="index">
          <span
            :class="
              artist.url
                ? 'cursor-pointer hover:underline hover:text-accent-600'
                : 'pointer-events-none'
            "
      ...
      <span
        ...
        @click="router.push(makeSongURL(song.path))"
      >
        ...
        <span
          v-if="
            ...
            song.artists &&
            song.album_artists &&
            !equals(song.artists, song.album_artists)
          "
          class="text-ls-400 dark:text-ds-500"
        >
