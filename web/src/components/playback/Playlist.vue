  ...
  <div class="flex flex-col py-8 pl-8 xl:pl-16 pr-8 bg-ls-0 dark:bg-ds-900">
    <div class="relative flex gap-2">
      ...
      <Transition appear name="slide">
        <div
          v-if="savingPlaylist"
          v-on-click-outside="cancelSavePlaylist"
          class="z-10 absolute right-0 -bottom-2 w-80 translate-y-full rounded-md bg-ls-0 dark:bg-ds-950 shadow-lg shadow-accent-600/20 dark:shadow-none dark:border dark:border-ds-800"
        >
          <form class="relative p-6 flex flex-col gap-4" @submit.prevent="savePlaylist">
            <InputText id="playlistName" v-model="playlistName" label="Playlist Name" autofocus />
            <Button
              type="submit"
    ...
    <div class="mb-8 flex items-center justify-between">
      <div class="flex gap-4">
        <Select v-model="playbackOrder" class="w-40 xl:w-48" :options="playbackOrderOptions" />
        <Button
          label="Shuffle"
          severity="secondary"
          size="base"
          icon="shuffle"
          @click="playback.shuffle"
        />
      </div>
      <!-- TODO tooltips -->
      <Switch
        v-model="preferences.playlistDisplayMode"
        :items="[
...
import { type PlaybackOrder, type PlaylistEntry, usePlaybackStore } from '@/stores/playback';
import { usePlaylistsStore } from '@/stores/playlists';
import { usePreferencesStore } from '@/stores/preferences';

const Stats = defineAsyncComponent(() => import('@/components/playback/Stats.vue'));
