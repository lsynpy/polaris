...
test("can set album art pattern", async ({ page }) => {
  ...
  const putRequest = page.waitForRequest(
    (request) =>
      request.method() === "PUT" && request.url().endsWith("/api/settings")
  );
...
test("can add and remove mount dir", async ({ page }) => {
  ...
  const waitForPut = () =>
    page.waitForRequest(
      (request) =>
        request.method() === "PUT" && request.url().endsWith("/api/mount_dirs")
    );
  ...
  await expect(
    page.getByTestId("location").getByRole("textbox").last()
  ).toHaveValue(location);
  await expect(
    page.getByTestId("name").getByRole("textbox").last()
  ).toHaveValue(name);
...
test("can add and remove user", async ({ page }) => {
  ...
  const postRequest = page.waitForRequest(
    (request) =>
      request.method() === "POST" && request.url().endsWith("/api/user")
  );
  ...
  const deleteRequest = page.waitForRequest(
    (request) =>
      ...
      request.url().endsWith(`/api/user/${username}`)
  );
